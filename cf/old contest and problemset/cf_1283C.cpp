...
{
    faster
    vector<int>dei_ni,pai_ni;

    int n,x;
    cin>>n;
    for(int i=1; i<=n; i++)
    {
        cin>>a[i],omap[a[i]]=i;///vec[i+1].pb(x);
        if(a[i]==0)
            dei_ni.pb(i);
    }
    x=dei_ni.size();
    ...
    ///cout<<x<<' '<<pai_ni.size()<<endl;
    sort(dei_ni.begin(),dei_ni.end());
    sort(pai_ni.begin(),pai_ni.end());
    for(int i=0,j=x-1;i<x;i++,j--){
        a[dei_ni[j]]=pai_ni[i];
        if(dei_ni[j]==pai_ni[i]){
            if(j+1<x)
                swap(a[dei_ni[j]],a[dei_ni[j+1]]);
            else
            {
                i++,j--;
                a[dei_ni[j]]=pai_ni[i];
